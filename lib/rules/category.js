module.exports = {
  ...
  filter: function(options, callback){
    if(this.categories.length > 0){
      this.core.inCategories(options.url, options.domain, this.categories, function(in_category){
    ...
    inCategories: function(url, domain, categories, callback){
      ...
      if(this.categories[url] && this.categories[url][category]){
        ...
        if(this.categories[url][category] instanceof Array){
          ...
          //Read from cache  
          callback(this.categories[url][category]);
        }    
      }else{
        if(this.redis){

          this.categories[url] = this.categories[domain] || {};
          this.categories[url][category] = [callback]; //put it into the callback queue

          this.redis.listContainsItems(categories, items, function(in_category){
            self.categories[url] = self.categories[url] || {};
